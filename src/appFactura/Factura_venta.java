...
//				SubstanceDefaultLookAndFeel.setCurrentWatermark("org.jvnet.substance.watermark.SubstanceMarbleVeinWatermark");//fondo por defecto
	...
	public void conectar(){
	    ...
	    {Class.forName("com.mysql.jdbc.Driver").newInstance();
		 ...
		 sentenciaven.executeQuery("select id_ven, NOMBRE_APELLIDO_VEN as elvendedor from vendedores order by elvendedor");
	...
	public void actualizarticulos(){
		try{
			sentenciaart.executeQuery("SELECT ID_ART,DESCRIPCION_ART,STOCK,PVP_ART FROM ARTICULOS ORDER BY ID_ART");
...
//	}
	...
	public void cargardetalle(){
		try{sentenciadet=conexion.createStatement();
		    limpiardetalle();
			sentenciadet.executeQuery("select facv_det.id_art,articulos.DESCRIPCION_ART,facv_det.pvp,facv_det.cant from facv_det inner join articulos on(facv_det.id_art=articulos.id_art) where num_facv='"+resultadocab.getString("num_facv")+"'");
    	...
    	try{int xfac=modelo.getRowCount();
    	    ...
    	    {   //ACTUALIZAR VENTAS DEL VENDEDOR
    			...
    			if (CboForma.getSelectedItem()=="CREDITO")
    			   {sentenciaupdate.executeUpdate("UPDATE CLIENTES SET SALDO_CLI=SALDO_CLI+"+Double.valueOf(TxtSaldo.getText())+" WHERE ID_CLI='"+resultadocli.getString("id_cli")+"'");
    			   }
    			//GUARDAR CABECERA

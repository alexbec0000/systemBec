...
public class f_listafacturas extends JDialog {
    ...
    public void conectar() {
        try {
            resultadoven = VendedorController.listarVendedores();
            cargarvendedores();
            ventaspornum();
        } catch (Exception e) {
    ...
    private void actualizalista(String condicion) {
        try {
            resultado = VendedorController.obtenerVendedores(condicion);
    ...
    public void ventaspornum() {
       actualizalista("where num_facv like '%" + TxtNumero.getText() + "%' order by num_facv");
    }

    public void ventasporcli() {
        actualizalista("where concat(clientes.RAZON_SOCIAL_CLI,' ',clientes.NOMBRE_APELLIDO_CLI) like '" + TxtCliente.getText() + "%' order by num_facv");
    }

    public void ventasporven() {
        actualizalista("where concat(vendedores.USUARIO_VEN,' ',vendedores.NOMBRE_APELLIDO_VEN) like '" + CboVendedor.getSelectedItem() + "%' order by num_facv");
    }

    public void ventasporfec() {
        actualizalista("where DATE_FORMAT(facv_cab.fecha,'%d/%m/%Y')= '" + TxtFecha.getText() + "' order by num_facv");
    }

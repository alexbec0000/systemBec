...
public class f_vendedores extends JFrame {
    ...
    private JTextField getTxtUsuario() {
        if (txtUsuario == null) {
            ...
            txtUsuario.setSize(new Dimension(452, 24));
            txtUsuario.setDisabledTextColor(Color.darkGray);
            txtUsuario.setLocation(new Point(131, 95));
            ...
            txtCedula.addFocusListener(new java.awt.event.FocusAdapter() {
                public void focusLost(java.awt.event.FocusEvent e) {
                    if (txtCedula.getText().length() != 10 & txtCedula.getText().length() != 13) {
                        JOptionPane.showMessageDialog(null, "Ingrese 10 números para la Cédula o 13 para el RUC", "Error de ingreso", 2);
                        txtCedula.setText("");
                    }
                }
            });
            ...
            bGuardar.addActionListener(new java.awt.event.ActionListener() {
                public void actionPerformed(java.awt.event.ActionEvent e) {
                    ...
                    if (txtNombre.getText().trim().length() == 0) {
                        ...
                        JOptionPane.showMessageDialog(null, "¡Ingrese el usuario del Vendedor!", "Error de almacenamiento", 2);
                        txtUsuario.requestFocus();
                    } else if (txtDireccion.getText().trim().length() == 0) {
                        JOptionPane.showMessageDialog(null, "¡Ingrese la Dirección del Vendedor!", "Error de almacenamiento", 2);
                        txtDireccion.requestFocus();
                    } else if (txtCedula.getText().trim().length() == 0) {
                        JOptionPane.showMessageDialog(null, "¡Ingrese la Cédula o RUC del Vendedor!", "Error de almacenamiento", 2);
    ...
    private JButton getBCancelar() {
        if (bCancelar == null) {
            ...
            bCancelar.setToolTipText("Cancelar información");
...
//				SubstanceDefaultLookAndFeel.setCurrentWatermark("org.jvnet.substance.watermark.SubstanceMarbleVeinWatermark");//fondo por defecto
    ...
    private void initialize() {
        ...
        this.setIconImage(Toolkit.getDefaultToolkit().getImage(getClass().getResource("/recursos/GENTES.JPG")));
    ...
    private JPanel getJContentPane() {
        if (jContentPane == null) {
            ...
            jLabel4 = new JLabel();
            jLabel4.setIcon(new ImageIcon(getClass().getResource("/recursos/0162iconos-clubzonanokia.com.png")));
    ...
    private void actualizar() {
        try {
            ...
            txtNombre.setText(resultado.getString("NOMBRE_APELLIDO_VEN"));
            txtUsuario.setText(resultado.getString("USUARIO_VEN"));
            txtDireccion.setText(resultado.getString("DIRECCION_VEN"));
            txtCedula.setText(resultado.getString("CED_RUC_VEN"));
            txtTelefono.setText(resultado.getString("TELEFONO_VEN"));
            txtCelular.setText(resultado.getString("CELULAR_VEN"));
            cboSexo.setSelectedItem(resultado.getString("SEXO_VEN"));
            txtEmail.setText(resultado.getString("EMAIL_VEN"));
            txtFecnac.setText(resultado.getString("FEC_NACIMIENTO_VEN").substring(8) + "/" + resultado.getString("FEC_NACIMIENTO_VEN").substring(5, 7) + "/" + resultado.getString("FEC_NACIMIENTO_VEN").substring(0, 4));
            txtFecing.setText(resultado.getString("FECHA_INGRESO_VEN").substring(8) + "/" + resultado.getString("FECHA_INGRESO_VEN").substring(5, 7) + "/" + resultado.getString("FECHA_INGRESO_VEN").substring(0, 4));
        ...
        try {
            if (xregistros == 0) {
                ...
                if (cadnuevo.length() == 5) {
                    txtCodigo.setText("");
                    JOptionPane.showMessageDialog(null, "¡No se puede generar el código. Se ha superado la capacidad del sistema!", "Desbordamiento de valores", 0);
                    System.exit(0);
                }
            }
        } catch (Exception g) {
    ...
    public void eliminar() {
        try {
            ...
            if (resultadoventas.getInt("CUANTOS") > 0) {
                JOptionPane.showMessageDialog(null, "¡Este Vendedor no puede ser eliminado. Tiene registros vinculados en la Base de Datos!", "Error de eliminación", 2);
            } else {
                sentencia = conexion.createStatement();
                sentencia.executeUpdate("DELETE FROM VENDEDORES WHERE ID_VEN='" + txtCodigo.getText() + "'");
                JOptionPane.showMessageDialog(null, "¡Los datos del Vendedor han sido eliminados!", "Eliminación de datos", 1);
                sentencia.executeQuery("SELECT * FROM VENDEDORES ORDER BY ID_VEN");
                resultado = sentencia.getResultSet();
                if (resultado.next() == false) {
                    xregistros = 0;
            ...
            JOptionPane.showMessageDialog(null, "¡Ha ocurrido un error al intentar eliminar los datos del Vendedor!", "Error de eliminación", 2);
            return;
        }
    }
    ...
    private void guardar() {
        try {
            ...
            sentencia = conexion.createStatement();
            if (nuevoreg == true) {
                sentencia.executeUpdate("INSERT INTO VENDEDORES (ID_VEN,NOMBRE_APELLIDO_VEN,USUARIO_VEN,DIRECCION_VEN,CED_RUC_VEN,TELEFONO_VEN,CELULAR_VEN,SEXO_VEN,FECHA_INGRESO_VEN,FEC_NACIMIENTO_VEN,EMAIL_VEN,SUE_VEN,VENTAS,LABORA) VALUES('" + txtCodigo.getText()
                        + "','" + txtNombre.getText() + "','" + txtUsuario.getText() + "','" + txtDireccion.getText() + "','" + txtCedula.getText() + "','" + txtTelefono.getText()
                ...
                sentencia.executeUpdate("UPDATE VENDEDORES SET NOMBRE_APELLIDO_VEN='" + txtNombre.getText() + "',USUARIO_VEN='" + txtUsuario.getText() + "',DIRECCION_VEN='" + txtDireccion.getText()
                        + "',CED_RUC_VEN='" + txtCedula.getText() + "',TELEFONO_VEN='" + txtTelefono.getText() + "',CELULAR_VEN='" + txtCelular.getText()
                        + "',SEXO_VEN='" + cboSexo.getSelectedItem() + "',FECHA_INGRESO_VEN='" + fecingsql + "',FEC_NACIMIENTO_VEN='" + fecnacsql + "',EMAIL_VEN='"
    ...
    private void encuentra() {
        try {
            ...
            if (resultado.isAfterLast() == true) {
                JOptionPane.showMessageDialog(null, "¡El Vendedor no pudo ser localizado!", "Error de búsqueda", 2);
                resultado.last();
            }
            actualizar();
        } catch (Exception m) {
